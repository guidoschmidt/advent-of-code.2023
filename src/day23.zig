...
const Route = struct {
    ...
    pub fn findNext(self: *Route, gpa: Allocator, map: *[][]u8) !std.ArrayList(Route) {
...
fn printMap(map: *[][]u8) void {
    ...
    for (0..map.len) |x| {
        ...
        for(0..row.len) |y| {
            ...
            switch(v) {
                'X' => {
                    std.debug.print("{s}{c}{s}", .{ common.red, v , common.clear });
                },
                else => {
                    std.debug.print("{c}", .{ v });
                }
...
fn part1(gpa: Allocator, input: []const u8) anyerror!void {
    ...
    while (routes.items.len > 0) {
        ...
        var split_routes = try next_route.findNext(gpa, &map);
        for (split_routes.items) |route| {
            try routes.append(route);
        }
    }

    var longest_route: u16 = 0;
