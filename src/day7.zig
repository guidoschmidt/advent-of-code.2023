...
fn calculateRankTypePart2(hand: *const [5]u8) !Rating {
    ...
    if (joker_count > 0 and uniques.count() > 0) {
        ...
        while(key_it.next())|k| {
            var count = uniques.get(k.*).?;
            var count_highest_card = uniques.get(most_cards_key).?;
...
fn part2(input: []const u8) void {
    ...
    while (row_it.next()) |row| {
        var entry_it = std.mem.split(u8, row, " ");
        const hand = entry_it.next().?;
        const rating = calculateRankTypePart2(@ptrCast(hand.ptr)) catch Rating.NONE;
        const bid_str = entry_it.next().?;
        const bid = std.fmt.parseInt(u32, bid_str, 10) catch 0;
        cards.append(Card{ .bid = bid, .rating = rating, .hand = hand }) catch {
            std.log.err("\nCould not append Card to cards", .{});
...
pub fn main() !void {
    try common.runDay(allocator, 7, .PUZZLE, part1, part2);
}
